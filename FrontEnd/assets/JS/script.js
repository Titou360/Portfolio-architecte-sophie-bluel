  ...
  //  ---------------------------------------------------------
  //  | Management of the actions for edition mod             |
  //  ---------------------------------------------------------
  ...
  //  ---------------------------------------------------------
  //  | Connection with the API for "works" & "categories"    |
  //  ---------------------------------------------------------
  ...
  getWorks()
  	.then(data => {
  		...
  		// Here, categorie will be transform in filter
  ...
  //  ---------------------------------------------------------
  //  | Management to generating the figures				  |
  //  ---------------------------------------------------------
  ...
  //  ---------------------------------------------------------
  //  | Management for a fully dynamic modal                  |
  //  ---------------------------------------------------------
  function createModale() {
  	...
  	deleteFullGallery.addEventListener('click', () => {
  		const originalText = deleteFullGallery.textContent; // save the orginal content
  ...
  //  ---------------------------------------------------------
  //  | Management for the parts of the modal                 |
  //  ---------------------------------------------------------
  ...
  async function createAddPhotoModal() {
    ...
    <path d="M7.00004 46C7.23404 46 7.47004 45.918 7.66004 45.751L23.973 31.389L34.275 41.69C34.666 42.081 35.298 42.081 35.689 41.69C36.08 41.299 36.08 40.667 35.689 40.276L30.882 35.469L40.063 25.415L51.324 35.738C51.731 36.111 52.364 36.083 52.737 35.676C53.11 35.269 53.083 34.636 52.675 34.263L40.675 23.263C40.479 23.084 40.218 22.995 39.955 23.001C39.69 23.013 39.44 23.13 39.261 23.326L29.467 34.053L24.724 29.31C24.35 28.937 23.752 28.918 23.356 29.266L6.33904 44.249C5.92404 44.614 5.88404 45.246 6.24904 45.661C6.44704 45.886 6.72304 46 7.00004 46Z" fill="#B9C5CC"/>
  	...
  	submitButton.disabled = true; // Button is disabled at the beggining
  	...
  	function checkImageSizeAndExtension(selectedImage) {
  		if (selectedImage) {
  			...
  			if (selectedImage.size > imageSizeLimit) {
  				infoImgParagraph.classList.add("info-file-is-not-ok", "info-file");
  				infoImgParagraph.innerText = 'La taille de votre photo dépasse les 4 Mo autorisés.';
  			} else if (!validFormats.includes(selectedImage.type)) {
  				infoImgParagraph.classList.add("info-file-is-not-ok", "info-file");
  				infoImgParagraph.innerHTML = 'Format de fichier non pris en charge.<br> Veuillez sélectionner un fichier PNG, JPG ou JPEG.';
  			} else {
  				// If picture is valid, initializ the alert message and download image
  				blueRectangle.innerHTML = ''; // Reset the content in the blue rectangle
  				loadImage(selectedImage);
  			}
  		}
  	}

  	// Initial call to check button state on page load
  	checkFormFields();
  	...
  	function loadImage(event) {
  		...
  		if (selectedImage) {
  			...
  			// Add an EventListener for the new created input
